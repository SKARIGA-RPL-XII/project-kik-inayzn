...
import { Search, MessageCircle, Bookmark, User, LogOut, MapPin } from 'lucide-react';
...
export default function UserDashboard() {
    ...
    return (
        <div className="min-h-screen bg-white font-sans overflow-x-hidden">
            ...
            {/* HERO SECTION - ESTETIK DENGAN OVERLAY & VIDEO/IMAGE */}
...
<div className="relative z-20 flex flex-col items-center text-center px-4 max-w-4xl">
    ...
    {/* Search Bar Estetik Transparan (Glassmorphism) */}
    <div className="relative w-full max-w-2xl group px-4">
        <div className="relative overflow-hidden rounded-full p-[1px] bg-gradient-to-r from-white/30 to-transparent">
            ...
            {/* QUICK LINKS - DIBIKIN MELAYANG */}
            <div className="max-w-6xl mx-auto px-8 -mt-20 relative z-30 grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                    { label: 'Carikan Properti', icon: '🏠', desc: 'Konsultasi gratis' },
                    { label: 'Kategori Terlaris', icon: '🔥', desc: 'Paling banyak dicari' },
                    { label: 'Simulator KPR', icon: '📊', desc: 'Hitung cicilanmu' }
                ].map((item) => (
                    <button key={item.label} className="bg-white border border-slate-100 p-8 rounded-[2rem] shadow-2xl hover:shadow-emerald-900/10 hover:-translate-y-3 transition-all duration-500 text-[#1a432d] group">
                        <div className="bg-emerald-50 w-16 h-16 rounded-2xl flex items-center justify-center text-3xl mb-6 group-hover:bg-emerald-500 group-hover:rotate-12 transition-all duration-500">
                            {item.icon}
                        </div>
            ...
            <div className="max-w-7xl mx-auto px-8 py-24">
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-4">
                    ...
                    <Link href="/products" className="text-emerald-700 font-bold hover:text-emerald-500 transition-colors flex items-center gap-2 group">
                        Lihat Semua <span className="group-hover:translate-x-2 transition-transform">&rarr;</span>
                    </Link>
                </div>
