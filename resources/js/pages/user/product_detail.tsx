                    ...
                    <div className="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-xl border border-slate-100">
                        ...
                        <div className="space-y-8 mb-10">
                            {product?.ulasans?.map((rev) => (
                                <div 
                                    key={rev.id} 
                                    id={`ulasan-${rev.id}`} // Hook ID untuk navigasi notifikasi
                                    className="space-y-4 scroll-mt-24 transition-all duration-500"
                                >
                                    {/* Box Ulasan Utama */}
                                    <div className="bg-slate-50/80 p-6 rounded-[2rem] border border-slate-200">
                                        <div className="flex justify-between items-start">
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <button 
                                                    onClick={() => handleReplyClick(rev.id, rev.user?.username || 'User')}
                                                    className="text-xs font-black uppercase text-emerald-600 hover:text-emerald-800"
                                                >
                                                    Balas
                                                </button>
                                            </div>
                                        ...
                                        {rev.admin_reply && (
                                            <div className="mt-4 p-4 bg-emerald-50 border-l-4 border-emerald-500 rounded-xl">
                                                <div className="flex items-center gap-2 mb-1">
                                                    <span className="bg-emerald-600 text-white text-[10px] px-2 py-0.5 rounded font-black uppercase">Admin</span>
                                                </div>
                                                <p className="text-slate-800 text-sm font-bold">{rev.admin_reply}</p>
                                            </div>
                                        )}
                                    </div>

                                    {/* Box Balasan User (Threaded) */}
                                            ...
                                            <div className="bg-white p-4 rounded-[1.5rem] border border-slate-100 shadow-sm w-full">
                                                <div className="flex justify-between items-center mb-1">
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        <button 
                                                            onClick={() => handleReplyClick(rev.id, reply.user?.username || 'User')}
                                                            className="text-[10px] font-black uppercase text-emerald-600 hover:underline"
                                                        >
                                                            Balas
                                                        </button>
                                                        {reply.user_id === auth.user?.id && (
                                                            <button onClick={() => handleDelete(reply.id)} className="text-slate-300 hover:text-red-500 transition-colors">
                                                                <Trash2 size={14} />
                                                            </button>
                                                        )}
                                                    </div>
                        ...
                        <div id="input-ulasan" className="bg-slate-50 p-6 md:p-8 rounded-[2.5rem] border-2 border-dashed border-slate-200 focus-within:border-emerald-500 transition-all">
                            ...
                            <textarea 
                                ...
                                placeholder={replyTo ? `Balas @${replyTo.username}...` : "Bagaimana pengalaman Anda?"}
                                value={comment}
                            ...
                            <button 
                                ...
                                className={`px-10 py-5 rounded-2xl font-black text-lg flex items-center gap-3 transition-all ${
                                    isSubmitting ? 'bg-slate-300' : 'bg-[#1a432d] text-white hover:bg-emerald-900 shadow-md active:scale-95'
                                }`}
                            >
                                <Send size={20} /> 
